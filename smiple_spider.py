...
# # -*- coding: utf-8 -*-
# # @Time    : 2020-01-19 19:04
# # @Author  : July datetime
import os
import sys
import requests
import csv
...
def city_spider():
    ...
    cities_json = ret['data']['domesticcity']['cities']
    need_list = []
...
def xc_spider(keyword):
    ...
    data = {"ver": "8.3.2", "debug": False, "pidx": 1, "isFirstScreen": True,
            ...
            "distids": "", "sort": 1, "psize": 1000, "isneedf": True, "gsscid": "2", "gslcid": None, "scenes": 12,
    ...
    for activityinfo in activityinfos:
        ...
        try:
            ...
            cityname = ''
        try:
            mprice = activityinfo['mprice']  # 价格
        except:
            mprice = ''
        try:
            price = activityinfo['price']  # 价格
        except:
            price = ''
        try:
        ...
        try:
            ...
            avdate = ''
        need = [id, name, cityname, mprice, price, saletotal, avdate, keyword]
...
def start1(city):
    ...
    for i in range(0, len(keywords_list)):
        ...
        save_data(data=data, filename=city)
        print(f'景点总数{len(keywords_list)}，第{i+1}个景点数据存储成功~~~')


def start2():
    path = get_path('景点.txt')
    try:
        ...
        for keyword in keyword_list:
            data = xc_spider(keyword)
            save_data(data=data, filename='景点语音播报')
...
if __name__ == '__main__':
    while True:
        flag = input('根据“城市名”爬取请输入1，根据“景点.txt”文件爬取请输入2：')
        if flag == '1':
            c = input('请输入城市名：')
            start1(c)
        elif flag == '2':
